...
async function publish() {
  ...
  move
    .createObjectAndPublishPackage({
      ...
      extraArguments: [`--private-key=${process.env.NEXT_MODULE_PUBLISHER_ACCOUNT_PRIVATE_KEY}`,`--url=${aptosSDK.NetworkToNodeAPI[process.env.NEXT_PUBLIC_APP_NETWORK]}`, `--skip-fetch-latest-git-deps`],
    })
    .then((response) => {
      const filePath = ".env";
      let envContent = "";
